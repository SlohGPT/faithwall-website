# FaithWall SEO Strategy — v2 (2026-08-22, data notes through 2026-09-14)
...
3. **bible-study-tools-ios — the growth frontier, now on the board (status flipped 2026-08-31).** The "best/free Bible app for iPhone" hub is the largest named-query demand pool we touch and has risen for four consecutive cycles. It has now crossed from deep page 2 to the page-1 threshold and is taking real clicks for the first time — the largest single positional gain the site has recorded. This remains the #1 content priority: the crossing proves the depth method works, and the distance still to travel (page-1 edge to top-5) is the part §1 says is authority-bound, which makes this pillar simultaneously the #1 beneficiary of the §8 layer. **Update 2026-09-07:** the crossing held and compounded rather than settling back — the hub is now among the site's leading click sources and its head term sits at the page-1 edge instead of three pages back. One cost surfaced alongside it: the pillar's dedicated study page has lost study-intent phrasings to the hub and fallen well back, so the pillar's next move is a scoping decision between those two pages, not more hub depth. It is queued as a proposal, not a merge — the study page still has a §7 brief, and handing a named cluster's identity to a page that has only just reached page 1 itself is not a call to make on one window of data. **Update 2026-09-14 — scoping decided: re-scope, not merge.** The two head terms share almost nothing in their external results, so Google treats study and free-reading as separate intents; the study page's slide is a depth gap against its own promise (notes, highlights, commentaries), not a collision. It gets a depth-pass. The hub keeps the free-study phrasings it already ranks for — no trim until the rebuilt study page has been measured.
4. **prayer-life-iphone — adjacent flagship.** Head page in upper page 1. Consolidation, not expansion.
5. **faith-based-productivity — harvest only.** Three consecutive refreshes with no demand. No new posts; candidate for §5 prune-merge review. **Review completed 2026-09-07:** the cluster's two narrowest posts carry no visibility and no identifiable intent of their own across two consecutive windows, and their substance is already covered at greater depth by the focus-modes page — the one page in the cluster that converts. Consolidation into it is authorized. The remaining three posts hold distinct angles and stay. **Executed 2026-09-10:** both narrow posts now permanently redirect into the focus-modes page and their keywords are blocked; the cluster's consolidation is complete.

**Newly evidenced surface (2026-08-24): generator/tool demand.** The /random-bible-verse tool page surfaced a coherent query family the pillar set had never reached — generator-phrased searches for a random verse — and it is now by a wide margin the most query-legible page on the site, where our pillars are almost entirely anonymized. A SERP-overlap check confirms the intent is genuinely separate from the moat family, and the competing results are thin single-purpose tool sites rather than App Store listings. **Confirmed and enlarged 2026-09-07:** the generator-phrased family is now the single largest identifiable query block the site has, and the tool page meets it at low page 1 to page 2 — the clearest striking-distance position we own anywhere. This is not a sixth pillar and does not change the pillar ranking: it is one page holding one adjacent family, and the correct response is depth on the page that already owns it, not new URLs. Its value beyond its own clicks is that it is the one surface where we can actually *measure* what we rank for. **Update 2026-09-14:** the 09-02 depth-pass is the cleanest measured win the depth method has produced — within its first week the page climbed several places to the page-1 edge on the generator head phrasings and its clicks rose several-fold, while the family itself kept growing. The page already covers the randomizer/generator synonyms and answers the chapter-generator variant, so the next gains here are positional, not coverage.

## 3. The Strategic Plays

### Play 1 — Win the "best Bible app" hub (page 2 → page 1)

Unchanged goal, upgraded method — and as of 2026-08-31 the goal in its original form is met. The hub (/blog/best-free-bible-app-iphone) has climbed on every query in the family for four cycles and has now reached page 1, where it converts for the first time; the named head terms in the family moved up roughly a full page each. **The play is not closed — it is re-aimed:** the remaining target is top-5, and the on-page half of that is the depth work below while the other half is authority (§8). Treat page-1 arrival as evidence the method works, not as a finish line. **Update 2026-09-14:** the 09-04 depth-pass compounded it — the generic "best Bible app for iPhone" head term moved from the page-1 edge into mid page 1 and took its first clicks, and the free-app head term edged up again.
...
/best-bible-verse-lock-screen-apps and the moat family sit at or near top of page 1 with several #1 variants. Click-chasing on the page's *superlative* phrasings is closed. **Amended 2026-09-07 — one click gap reopened, narrowly.** A large block of *descriptive* demand became visible for the first time this cycle: people describing the behaviour they want (an app that puts, shows, or displays Bible verses on the lock screen) rather than asking which app is best. The page meets that block at the bottom of page 1, where it converts nothing, while holding top-5 on the superlative wording. That is a coverage gap, not the authority gap of §1, and it is currently the largest legible click opportunity on the site. Scope it as passage-level coverage of the descriptive wording — it is explicitly not a licence to reopen title or meta work on this page, which is performing. Remaining work: monthly materially-true dated refresh (freshness is the strongest cheap AI-citation lever we have), a `depth-pass` toward comprehensive coverage of the lock-screen-app question, and copycat disambiguation maintenance. The vs-pages are judged as comparison-citation surfaces, not click engines. **Update 2026-09-14:** the 09-05 depth-pass improved the page's average position and click rate in its first week, but the descriptive block itself has not moved, so the passage-level coverage item stands as scoped.

### Play 3 — CTR: concluded, with one standing finding

The homepage title/meta and rich-results work shipped and the brand-query finding stands: the two-word "faith wall" query is structurally unconvertible from the web result (decor-ambiguous SERP); the one-word "faithwall" spelling ranks #1 and converts strongly. **No further homepage markup/title passes.** **Verdict closed 2026-08-24:** after two full windows with the branded rich-result markup live, the two-word query's position and conversion are unchanged and its impressions have stopped growing, while the one-word spelling has risen from absent to converting a large share of what it sees. The diagnosis is confirmed and the play is finished — this is a SERP-context problem, not a markup problem, and it is now closed rather than pending. The fix for two-word brand loss is off-site disambiguation and growing one-word branded demand — i.e., Play 4. **Footnote 2026-09-07:** the two-word query has since registered its first conversions, at a token rate, while its impressions shrank further; the one-word spelling converts an order of magnitude better and is now the strongest-converting query on the site. Nothing here changes the verdict — a trickle of conversions on a shrinking, decor-ambiguous SERP is the diagnosis, not a counter-example — and the play stays closed. The anonymized test from v1 remains binding: no `ctr-fix` on pages whose demand carries no identifiable queries.
...
- **Effort signals from a device-less run (mandatory — added 2026-08-30 after the p21 incident).** The cloud agent has no iPhone and no simulator. It must **never** write a first-person testing, timing, or device-behaviour claim: no "we tested", "in our test", "we timed", "we backed out to the home screen", no "confirmed background playback", no observed timings or battery/offline results. When a page would need device evidence, it satisfies the two-signal rule above from what it can honestly source — App Store listing facts (price, size, version, release date, publisher), publisher documentation quoted and attributed, iOS setting paths, a named-author judgment call with stated reasoning, or an original screenshot **only if one already exists in the repo** — and writes the device test that is actually needed as a dated line in `TODO-KAROL.md` for Karol, instead of inventing it. A page that cannot honestly reach two signals ships with one plus that TODO line; it does not manufacture the second. **This rule outranks the two-signal minimum.** An unverifiable first-person claim is a hard merge-gate FAIL, and a failed branch blocks every subsequent autopilot run — the p21 branch cost 08-23 → 08-30: eight days, zero output.
