...
2. **christian-app-comparisons — the money layer.** Home of /best-bible-verse-lock-screen-apps (the strongest page on the site), the root FaithWall-vs-X pages, and app roundups. Status: fastest-growing surface, and the money page is now the site's leading page on both impressions and clicks outright — it also converts better than any other high-impression page we own. Keep pushing the roundup to top-3 and lift the second tier (the vs-pages and the 2026 roundup still rank on page 1 while converting almost nothing). Note that /faithwall-vs-youversion's weak conversion is *not* a fixable CTR problem: its visible demand is YouVersion-navigational, and those searchers were never going to click us. Judge it as a comparison-citation surface.
3. **bible-study-tools-ios — the growth frontier.** Top-of-funnel volume via the "best/free Bible app for iPhone" hub, still on page 2 but climbing hard — the hub and every query in its family gained substantial ground again this cycle, some by ten places or more. The definitive-hub rebuild (Play 1) shipped 2026-08-08; the first window that measures it closes at the next weekly refresh. Largest unconverted demand pool we touch.
...
5. **faith-based-productivity — harvest only.** Pages rank fine when shown, but the keyword set shows essentially no search demand — re-confirmed again at the 2026-08-10 weekly refresh, where every page in the cluster including its pillar sat at or near the very bottom of the site's visibility despite holding page-1 positions. Two consecutive refreshes with no movement. No new posts here until demand appears; spend those slots on clusters 1–3.
...
**Status.** The rebuild shipped 2026-08-08 (queue item p2); the first data window that includes it closes at the refresh after next-day. Two intended effects are already visible in the pre-rebuild baseline and must be re-checked: "best bible apps 2026" has started surfacing on this hub rather than only on the study page (the §7 reassignment working), and the audio-Bible family's demand grew several-fold while remaining on this hub, validating the decision to hold audio intent here and link the spoke rather than move the target.
...
**Status.** The hardening pass (p1) shipped 2026-08-06 and the internal-link build (p3, thirteen in-body links from the two aligned clusters) shipped 2026-08-09; the page is no longer link-starved. Neither is measurable yet — both landed at or after the close of the data window, during a period when the page's position and impressions were already climbing steeply on their own. The next refresh must separate our effect from that momentum rather than credit the whole delta to the work. One durable finding: the App-Store-phrase navigational query is this page's largest named impression source and converts nothing even as its position improves. That is intent-structural — it is an ASO and disambiguation surface, not a click target, and no further click-chasing work should be aimed at it.
...
**The anonymized test is now binding, not advisory.** Before queueing any `ctr-fix`, check what share of the target page's impressions carry an identifiable query. Where that share is negligible, a title rewrite is unmeasurable by construction and the item must be an AEO-hardening pass instead — this test retired one queued title-churn item at the 2026-08-10 refresh. The brand query remains the exception that proves the rule: it is fully legible, holds a top-4 position, produces no clicks at all, and is by a wide margin the largest single unconverted pool on the site.
